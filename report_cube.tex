...
So was one of the experiments to create a magnetic field rotating in the cartesian x-z-plane. The purpose behind performing this experiment was the alignment of the Hall sonde in the cube. Because a magnetic field detector based on a Hall bar is only able to detect components of a magnetic field perpendicular to the plane of the bar, a sonde parallel to the x-y-plane would result in a maximal amplitude of the recorded sine wave of the Hall voltage. This was performed by entering a sine and cosine signal for the $x$ and $z$ coordinate in the LabView program, that converted these signals according to the transfer algorithm displayed in equations \ref{matrix rot} the appendix. These transformed signals were finally used to generate the desired field in the cube.
